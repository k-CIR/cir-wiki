...
* Further technical specifications are found [here]({{ manuals_path }}/Big_Knob_Studio-Studio+_OM.pdf).

## Issues

??? failure "There is no sound to the MSR"
    Sound from the Stimulation Computer, Audiofile, or talkback does not appear inside the MSR.
    ...
    !!! success ""
        ...
        - For sound tubes: check that the sound tube amplifier is switched on. The sound tube amplifier is located in the Stimulus Cabinet. Switch it on if it is switched off.
    ...
    !!! warning ""
        Do not switch on the sound tube amplifier if a participant has the sound tubes in their ears.

??? failure "There is no sound from the MSR/I cannot hear the participant"
    There is no sound from the MSR, and I cannot hear if the participant is saying anything.
...
??? failure "Nothing is working, is there another way?"
    You can use the built in Triux system intercom.
