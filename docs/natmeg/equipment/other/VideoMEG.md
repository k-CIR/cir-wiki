...
> If you would unplug or switch camera make sure to restart the video recording software.

## Preparing measurements

### File structure
The videomeg computer sends tiggers to the OPM acquisition computer on port AI21. These trigger timestamps are also stored in the *.vid en *.aud files that are recorded on this computer. Upon conversion of the vid and aud files to an *.avi file, they are also burned into the avi video stream.
...
2. Start OPM data acquisition, check the presence of trigger, run your experiment.
3. Stop OPM acquisition
...
If no subject or task is defined, file will be saved with timestamp and run#.
