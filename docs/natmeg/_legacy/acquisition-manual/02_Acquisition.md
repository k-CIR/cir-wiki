...
<a id="how-to-restart-acquisition"></a>

??? failure "How to restart Acquisition?"
    Channels are not appearing when running Acquisition. Acquisition is giving errors about "lost connection" or "cannot connect to channels".

    !!! success ""
        In order do the following, if your problem keeps appearing then proceed to the next step; otherwise do not proceed:
